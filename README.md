...
Alongside it, a second cache layer is shared machine-wide across every project, worktree, and clone: `~/.cache/sveld/parse-cache-<toolchain>.json` (or `$XDG_CACHE_HOME/sveld/` if set), one file per `sveld`+Svelte version combination so two worktrees on different versions never contend for the same file. Unlike the project-local cache above, which is keyed by file path, this one is keyed by the source file's content hash, so a component with byte-identical source hits it regardless of which project parsed it first — including a component's first-ever parse in a brand-new clone. A component that reaches outside its own source via `@extendProps`/`@extends` is never written to it, since its parsed output isn't a pure function of its own hash. It's on by default whenever `cache` is enabled; pass `globalCache: false` (or `--global-cache=false`, or `--global-cache false`) to disable just this layer. Both layers are best-effort: a read-only or missing home directory never fails a run.

Anything with write access to `~/.cache/sveld` can plant an arbitrary parsed result under a real source hash, and a hit skips parsing entirely — trust it the way you'd trust any other build cache with write access to your home directory. That's a non-issue on a single-user machine (the common case: your own laptop, your own worktrees), but treat it as trusted-writer input on a shared or persistent CI runner; see the security note in [CI: priming the cache](#ci-priming-the-cache).
...
Flags are kebab-case: `--entry`, `--glob`, `--types`, `--json`, `--markdown`, `--fail-fast`, `--dry-run`, `--cache`, `--global-cache`, `--resolve-types`, `--check-examples`, `--report-diagnostics`, `--strict`, `--check`, `--types-format`, `--quiet`, `--stdout`, `--format`. The camelCase spellings `--resolveTypes` and `--checkExamples` still work as deprecated aliases for compatibility with existing scripts. `--entry`, `--cache`, `--global-cache`, `--check`, and `--types-format` take their value either as `--flag=value` or as a separate `--flag value` argument (`sveld --entry src/index.js` and `sveld --entry=src/index.js` are equivalent); if the next argument starts with `--` it's not consumed as a value, so `--cache`, `--global-cache`, and `--check` fall back to their default (`--global-cache` to enabled) and `--entry` and `--types-format` report a usage error naming the flag. Boolean flags (`--json`, `--glob`, `--strict`, and the like) never consume a following argument. An unrecognized flag (e.g. `--markdwon`) prints `Unknown flag: --markdwon` to `stderr`, exits `1`, and skips generation; when a close match exists it appends a suggestion, e.g. `Unknown flag: --markdwon Did you mean --markdown?`. sveld takes no positional arguments, so any non-flag argument errors the same way.
...
- The global cache (`~/.cache/sveld/`, or `$XDG_CACHE_HOME/sveld/` if set) is more effective to prime for CI specifically because it's shared across every job that restores the same cache entry — e.g. every job in a workflow run, or every run on a branch, depending on how your CI scopes cache restores.

Use an exact cache key, not a prefix fallback: a global-cache hit skips parsing and serves whatever was cached under that hash verbatim, so it's trusted-writer input to your `.d.ts`/docs output the same way any restored build cache is, and a fuzzy prefix match widens what gets trusted for the sake of a warmer (but less predictable) cache. That distinction matters most on a persistent or self-hosted runner shared across branches or forks; a normal ephemeral hosted runner scoped to your own repo is close to the same trust level as your own laptop. The cache file itself is already partitioned by toolchain version (`parse-cache-<sveld+svelte version>.json`), so an upgrade can't serve stale entries regardless of the CI key — you don't need to fold a version hash into the key to get that; a key on something that tracks source changes (a lockfile hash) is enough.
...
- **`globalCache`** (boolean, optional, default: `true`): Also share parsed output across every project/worktree/clone via a machine-wide, content-hash-keyed cache at `~/.cache/sveld/parse-cache-<sveld+svelte version>.json` (under `$XDG_CACHE_HOME/sveld/` if set). On by default whenever `cache` is enabled; pass `false` to disable just this layer. No-op when `cache` is `false`. Also available as `--global-cache` / `--global-cache=false` / `--global-cache false`. See [Persistent parse cache](#persistent-parse-cache-cache-globalcache).
